...
def create_reference_nodes(
    ...
    text_splitter = (
        SentenceSplitter(**ref_split_kwargs) if ref_split_kwargs else SentenceSplitter() # chunksize=512 overlap=20
    )

    def _split(nodes: List[RerankResult]) -> List[RerankResult]:
        idx = 1
        for node in nodes:
            text_splits = text_splitter.split_text(node.document["text"]) # split text into chunks 每512分一次
