...
# chroma启动脚本
# chroma run --path ./chroma_db_oncology
# 这时候我们的chroma DB 客户端已经在运行了
# 然后开始embed操作 如 python embed.py --to_embed meditron
...
def embed_chroma(
    ...
    for path_key in to_embed:
        ...
        # 根据 mediron来查找具体的sub_chunk_sizes的列表
        ...
        chroma_db_collection = CollectionWrapper( # 我们这个传了chroma_collection 也就包含了我们定义的embedding_function
        ...
        try: # 到了我们的CollectionWrapper类里面 然后开始进行数据转换documents-->nodes-->嵌入向量和添加到chroma向量数据中
            chroma_db_collection.add_documents(data_file_path=data_file_path) # 这里读入单个路劲 格式为：['~medition.jsonl]
